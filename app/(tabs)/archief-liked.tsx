...
import { api } from '@/lib/api'; // ⬅️ use the same API client as EventActions
...
export default function ArchiefLiked() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      // ⬇️ use the same API client as the like/going toggles
      const data: any = await api.get(`/events/mine/list?userId=${encodeURIComponent(uid)}&status=like&page=1&limit=100`, undefined, 25000, 1);
  ...
  if (!items.length) {
    return (
      <SafeAreaView style={styles.safeArea}>
        <View style={styles.center}>
          <Text style={styles.title}>Geen gelikete events</Text>
          <Text style={styles.muted}>Like een event op de hoofdpagina om het hier te zien.</Text>
        </View>
      </SafeAreaView>
    );
  }
