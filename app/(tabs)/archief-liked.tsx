...
export default function ArchiveLikedScreen() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      setLoading(true);
      const res = await fetch(`${base}/events/mine/list?userId=${encodeURIComponent(userId)}&status=like&page=1&limit=100`);
      if (!res.ok) throw new Error('Failed to load liked events');
      const data: MineResponse = await res.json();
      setItems(Array.isArray(data.items) ? data.items : []);
      setErr('');
    } catch (e: any) {
      setErr(e?.message ?? 'Error loading liked events');
    } finally {
      setLoading(false);
    }
  }, [base, userId]);
  ...
  if (loading) {
    return (
      <View style={styles.center}>
        ...
        <Text style={styles.muted}>Loading your liked events…</Text>
      </View>
    );
  }
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      ...
      <FlatList
        ...
        ListEmptyComponent={<Text style={styles.muted}>You haven’t liked any events yet.</Text>}
      />
    </SafeAreaView>
  );
}
